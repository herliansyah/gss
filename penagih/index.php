...
/**************************
          PAGING
 **************************/
$sql = "SELECT COUNT(*) as CountRecord FROM tbl_penagih WHERE 1=1 $searchSql";
$rs = $oDb->execute($sql);
...
function doDelete(code) {
	...
	q = "Hapus Data ?";
	if (confirm(q)) {
		location.href = "penagih/delete.php?code="+code;
	}
}
function doEdit(code) {
	location.href = "?mod=penagih&cmd=edit&id="+code;
}
function doPrint(){
			win = window.open('penagih/print.php','print','menubar=yes,toolbar=no,location=no,status=yes,resizable=yes,scrollbars=yes');
}
</script>

<div id="sectionHeader">Data penagih > Index</div>
...
<br>
		...
		<table border="0" cellpadding="2" cellspacing="0" id="b1">
			...
			if ($rs->recordCount()) {
				...
				<tr class="<?=$class?>">
					<td><?=$idx?>.</td>
                    <td><?=$rs->fields['penagih']?><input type="hidden" name="code" id="code" value="<?=$row['id']?>" /></td>
					<td>
			...
			<br />
			<input type="button" name="btnAdd" value="Tambah" class="button" onClick="location.href='?mod=penagih&cmd=add'">&nbsp;
