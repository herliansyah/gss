...
if($_POST[pilihan]=='cetak'){
    ...
    header('Content-Disposition: attachment; filename=LaporanHutang.xls');
}
...
<link href="<?=$config['base_url']?>/inc/style_report.css" rel="stylesheet" type="text/css">
  	<div id="sectionReportTitle" align="center"><b>Laporan Hutang Pelanggan Tahun <?=$tahun?> sampai dengan Bulan <?=number_to_bulan($bulan)?></b></div>
	<table border="0" width="100%" id="b3" cellspacing="0">
		...
		<tr class="<?=$class?>">
            ...
            <td><?= $tarif ?></td>
            <td><?
				$sqlb = "select * from tbl_bulan where bulan not in (SELECT bulan
