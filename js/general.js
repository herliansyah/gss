...
document.getElementById('customerDiv0').innerHTML='<input type="hidden" name="customer" id="cust" value="'+cid+'">'+cname;
...
//The pageSet element
function setPages(npages,page,nama_file){
	...
	switch(pset){
	  ...
	  case 1:
		...
		for(i=1;i < pvis+1 && ((pset-1)*pvis+i)<npages+1;i++){
	  		...
	  		tdata.innerHTML = '<a href="admin.php?action='+nama_file+'&page='+((pset-1)*pvis+i)+'">'+((pset-1)*pvis+i)+'</a>';
